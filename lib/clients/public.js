...
const DEFAULT_TIMEOUT = 11 * 1000; // 11 sec

class PublicClient {
  constructor(apiURI = 'https://api.abucoins.com', options = {}) {
    this.productID = 'BTC-USD';
    if (apiURI) {
      apiURI = arguments[1] || 'https://api.abucoins.com';
    }
  ...
  _deprecationWarningIfProductIdMissing(productID, caller) {
    if (!productID || typeof productID !== 'string') {
      process.emitWarning(
        `\`${caller}()\` requires a product ID as the first argument. ` +
